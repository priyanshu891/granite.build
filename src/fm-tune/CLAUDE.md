...
- **Granite `documents` differ by generation**: `apply_chat_template(..., documents=[...])` never raises when the template ignores the kwarg — it renders a valid prompt with the RAG context silently missing. Granite 3.x reads `document['doc_id']`/`['text']` (doc_id required); Granite 4.0/4.1 do `document | tojson` into a `<documents>` block with **verbatim keys** (so `doc_id` leaks into the prompt); **Granite 4.2 removed `documents` entirely** (moved to ChatML). `model_type` is `granite` for both 4.1 and 4.2, so detect via `tokenizer.chat_template` introspection, not model names. The SFT drivers forward the `documents` column blindly, so a 4.2 model + `documents` column trains on context-free prompts. Separately, **4.2 defaults to `enable_thinking=True`**, ending the prompt in an open `<think>` block — training a structured target (JSON verdict, NLI label, atom list) against that teaches the model to emit it as reasoning content; pass `enable_thinking=False` on templates that accept it. Fix: `autotune/tools/_chat_utils.py` (`detect_document_style`, `render_chat_input`) picks the document shape per template, inlines documents into a system message when unsupported, and gates the thinking kwarg. Shared by all three dataset builders (factuality/atomizer/nli), whose prompt texts (guardian/atomizer/NLI) live in the same module. The three SFT drivers render the template themselves for `chat`-format datasets, so they gate `enable_thinking` the same way (`_thinking_kwargs` in each driver, override via `disable_thinking: false` in `training_config`) — otherwise a chat dataset on 4.2 trains its target inside an open `<think>` block. See `docs/dataset-sft.md`.
...
- **Trial log attribution under concurrency**: `BufferedLogHandler.emit()` used to stamp every record with the handler's single mutable `self.trial_id`, which `CustomLoggerCallback` overwrote per Tune event. In the driver process *all* trials' output converges on that one root-logger handler — Ray runs `log_to_driver=True`, so worker stdout is forwarded into the driver, where `main.py` swaps `sys.stdout` for `PrintLogger` -> `logging`, and those lines arrive on Ray's log-forwarding *thread*, racing the Tune control thread. Result: trial A's logs stored under trial B. Fix: attribution is per-record — an explicit `extra={"trial_id": ...}`, else a `contextvars` id set by `logging_service.trial_context()` (used by the Tune callbacks, so a callback claims only its own records), else the handler default. The handler default is set **only in worker processes** via `logging_setup.bind_trial_id()`, which every driver calls once it knows its trial: one worker process serves one trial, so it can label at the source for every thread (HF Trainer, dataloaders). Never set a process-wide trial id in the driver. `logging_setup.bridge_runtime_env()` ships `AUTOTUNE_JOB_ID`/`AUTOTUNE_ENDPOINT_URL` to workers as a job-level Ray `runtime_env` so off-node workers can build their own handler (they don't inherit the driver's environ); it returns `{}` when the bridge is off, leaving `ray.init()` untouched by default. Related: `flush()` now detaches the buffer under the lock before the blocking send, so records emitted mid-POST aren't discarded by the post-send reset and a slow endpoint doesn't stall every thread that logs.
