...
"""Build an SFT dataset that teaches a model to classify an NLI relationship.

The input is a JSON file holding a top-level list of raw NLI records, each with:

- ``premise``: the premise text (text A).
- ``hypothesis``: the hypothesis text (text B).
- ``label``: one of ``entailment``, ``contradiction``, ``neutral``.

(This is exactly the format emitted by ``collect_nli_dataset.py``.)

Each record becomes one SFT example whose prompt asks the model to judge the
relationship between the premise and hypothesis and whose target is the JSON
``{"label": "<entailment|contradiction|neutral>"}`` — the same shape the eval
pipeline parses (``parse_nli_output`` / ``_reference_label``). The prompt block
is imported from ``autotune.tools._chat_utils`` so the training and eval prompts
can never drift. Two output shapes (matching ``build_atomizer_dataset.py``):

- ``formatted``: ``input`` is a string rendered through the model's chat
  template; ``output`` is the label JSON string.
...
``raw_nli.json`` yields ``raw_nli_formatted.jsonl`` and ``raw_nli_chat.jsonl``.
The ``--model`` argument is required when rendering the chat
...
Records whose output sequence (the label JSON) tokenizes to at least
``--max-output-length`` tokens (default 1024) are dropped, in both formats,
matching the length gate in ``build_atomizer_dataset.py``. Set
``--max-output-length 0`` to disable the gate (which also lets ``--format chat``
run without a model). The label target is tiny, so the gate is effectively a
no-op here, but it is kept for parity with the other builders.

Granite 4.2 moved to ChatML and defaults to ``enable_thinking=True``, which would
end the prompt inside an open ``<think>`` block and train the label JSON as
reasoning content. Thinking is therefore disabled unless ``--thinking`` is
passed; see ``autotune/tools/_chat_utils.py``.
...
# Canonical NLI labels accepted in the target (must match the eval-side parser).
_CANONICAL_LABELS = ("entailment", "contradiction", "neutral")


def _format_label(label: Any) -> Optional[str]:
    """Render the target JSON for a canonical label, or None if not canonical."""
    if not isinstance(label, str):
        return None
    canonical = label.strip().lower()
    if canonical not in _CANONICAL_LABELS:
        return None
    return json.dumps({"label": canonical})
...
def _make_record(
    ...
    premise = dp.get("premise")
    hypothesis = dp.get("hypothesis")
    label = dp.get("label")
    ...
    output_seq = _format_label(label)
    if output_seq is None:
        return None

    messages = get_nli_messages(premise, hypothesis)
    if fmt == "formatted":
        return {
...
def convert_split(
    ...
    the length gate in ``build_atomizer_dataset.py``.
    ...
    print(
        f"[nli/{fmt}] wrote {written} records to {output_path} "
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="build_nli_dataset",
        description="Build an SFT dataset for classifying NLI (entailment/contradiction/neutral).",
    )
    p.add_argument(
        "--input",
        required=True,
        help="Input JSON file (top-level list of {premise, hypothesis, label} records, "
        "e.g. the output of collect_nli_dataset.py).",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=1024,
        help="Drop records whose output (label JSON) tokenizes to this many tokens or more. "
    ...
    p.add_argument(
        ...
        "label JSON is not trained inside a <think> block.",
    )
    return p
